...
export default async function FightRequestsPage({ searchParams }: { searchParams: Promise<{ notice?: string; error?: string; q?: string }> }) {
  ...
  return <main className="fight-request-page"><SiteHeader /><section className="request-center-hero"><p className="eyebrow"><span /> Fighter access</p><h1>Fight requests</h1><p>Challenge a ranked fighter within {rankRange} positions of your official rank. Every matchup requires admin approval.</p><div><strong>#{fighter.rank ?? "–"}</strong><span>{fighter.name}</span><small>{fighter.wins}-{fighter.losses}-{fighter.draws}</small></div></section>{query.notice && <p className="request-page-notice">{query.notice}</p>}{query.error && <p className="request-page-error">{query.error}</p>}<section className="request-opponents"><div className="section-heading"><div><p className="eyebrow"><span /> Eligible opponents</p><h2>Choose your matchup</h2></div><form><input defaultValue={query.q ?? ""} name="q" placeholder="Search fighter name" type="search" /><button className="button button-ghost">Search</button></form></div>{!fighter.rank ? <div className="request-empty"><h3>You need an official rank.</h3><p>An admin must assign your fighter rank before requests become available.</p></div> : eligible.length ? <div className="opponent-grid">{eligible.map((opponent) => { const pending = pendingOpponentIds.has(opponent.id); return <article key={opponent.id}><span className="fighter-initial red-corner">{opponent.name[0]}</span><div><small>Rank #{opponent.rank}</small><h3>{opponent.name}</h3><p>{opponent.nickname ? `“${opponent.nickname}”` : "RFL Fighter"}</p><strong>{opponent.wins}-{opponent.losses}-{opponent.draws}</strong></div>{pending ? <b>Request pending</b> : <form action={requestFightAction}><input name="opponentFighterId" type="hidden" value={opponent.id} /><input name="returnTo" type="hidden" value="fight-requests" /><button className="button button-primary">Request fight</button></form>}<Link href={`/fighters/${opponent.id}`}>View profile →</Link></article>; })}</div> : <div className="request-empty"><h3>No eligible fighters found.</h3><p>Try another search or check again after rankings change.</p></div>}</section><section className="request-history"><div className="section-heading"><div><p className="eyebrow"><span /> Your activity</p><h2>Request status</h2></div></div>{requests.length ? requests.map((request) => { const other = request.requesterFighterId === fighter.id ? request.opponent : request.requester; return <article key={request.id}><div><strong>{other.name}</strong><small>{request.requesterFighterId === fighter.id ? "Sent" : "Received"} · {request.createdAt.toLocaleDateString("en-US")}</small></div><b className={`request-${request.status.toLowerCase()}`}>{request.status}</b>{request.fight ? <Link href={`/live/${request.fight.eventId}`}>{request.fight.event.title} · {request.fight.event.startsAt.toLocaleString("en-US")} →</Link> : <span>{request.status === "PENDING" ? "Awaiting admin review" : "No fight scheduled"}</span>}</article>; }) : <p>No requests yet.</p>}</section></main>;
}
