  ...
  for (let attempt = 0; attempt < 3; attempt += 1) {
    try {
      return await prisma.$transaction(async (tx) => {
        ...
        const pack = await tx.packDefinition.findUnique({ where: { id: input.packId }, include: { set: { include: { definitions: { where: { active: true } } } } } });
        ...
        const byRarity = new Map<CardRarity, typeof pack.set.definitions>();
        for (const rarity of ["COMMON", "RARE", "EPIC", "LEGENDARY"] as const) byRarity.set(rarity, pack.set.definitions.filter((card) => card.rarity === rarity));
        ...
        const selections = Array.from({ length: pack.cardsPerPack }, () => {
