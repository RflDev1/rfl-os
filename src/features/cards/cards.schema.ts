...
const cardFields = z.object({ setId: z.string().cuid(), fighterId: z.preprocess((value) => value === "" ? undefined : value, z.string().cuid().optional()), name: z.string().trim().min(2).max(80), subtitle: z.string().trim().max(120).optional(), rarity: z.enum(["COMMON", "RARE", "EPIC", "LEGENDARY"]), cardNumber: z.coerce.number().int().min(1).max(9999), imageUrl: z.preprocess((value) => value === "" ? undefined : value, z.string().url().startsWith("https://").max(1000).optional()), oneOfOne: z.preprocess((value) => value === "on" || value === true, z.boolean()) });
const supplyTransform = ({ oneOfOne, ...data }: z.infer<typeof cardFields>) => ({ ...data, maxSupply: oneOfOne ? 1 : null });

export const cardSchema = cardFields.transform(supplyTransform);
export const updateCardSchema = cardFields.extend({ active: z.preprocess((value) => value === "on" || value === true, z.boolean()) }).transform(({ oneOfOne, ...data }) => ({ ...data, maxSupply: oneOfOne ? 1 : null }));
