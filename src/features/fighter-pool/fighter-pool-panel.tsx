...
export function FighterPoolPanel({ initialState }: { initialState: PoolState }) {
  ...
  useEffect(() => {
    const refresh = async () => {
      const response = await fetch("/api/fighter-pool/state", { cache: "no-store" });
