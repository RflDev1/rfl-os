...
export const liveEventSchema = z.discriminatedUnion("type", [
  ...
  eventBaseSchema.extend({ type: z.literal("PLAYER_DISCONNECTED"), data: z.object({ minecraftUsername: z.string().trim().min(1).max(16), graceSeconds: z.number().int().min(1).max(1800) }) }),
