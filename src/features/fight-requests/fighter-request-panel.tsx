...
export async function FighterRequestPanel({ opponentFighterId, requestError, requestNotice }: { opponentFighterId: string; requestError?: string; requestNotice?: string }) {
  ...
  return <section className="fighter-request-panel"><div><p className="eyebrow"><span /> Fighter challenge</p><h2>Request this matchup</h2><p>Your rank: #{requester.rank} · Opponent rank: #{opponent.rank}</p></div>{requestNotice && <p className="request-notice" role="status"><span className="request-check" aria-hidden="true">✓</span>{requestNotice}</p>}{requestError && <p className="game-error">{requestError}</p>}{eligible ? <form action={requestFightAction}><input name="opponentFighterId" type="hidden" value={opponent.id} /><p>An admin will confirm rank eligibility and schedule the fight before either fighter is notified.</p><RequestFightButton sent={Boolean(pending)} /></form> : <p className="request-ineligible">This fighter is outside your permitted rank window.</p>}</section>;
}
